...
export default class ScenariiService {
  constructor ({ getServices, logger, dataHandler, notificationHandler, privateSocket, publicSockets }) {
    ...
    this.getScenarioInstances().then((instances) => {
      return Promise.all(instances.map(({ typeId, instanceId }) => {
  ...
  setScenarioData (scenarioData) {
    ...
    .then((scenario) => {
      ...
      instanceToUpdate.activated = scenarioData.data.activated // to trigger state listener broadcasting
      return instanceToUpdate
      // no need to set it in DB here : the triggered listener will make it! (else can be added twice...)
    ...
    return (event, value) => {
      ...
      switch (event) {
        case 'triggered':
          ...
          this.scenarioExecutions[value.executionId] = triggerChain
          break
        case 'aborted':
          value
          .then(() => {
        ...
        case 'activationChanged':
          this.logger.info(`Scenario ${instance.data.name} activation changed to ${instance.data.activated}`)
          // this.privateSocket.emit('activationChanged', instance) // no need for this now, but maybe later.
          ...
          break
        default:
          // TODO !0 unknown error case, and other cases to propagate
      }
    }
  }
}
