...
export default class ScenariiService {
  ...
  setScenarioData (scenarioData) {
    ...
    .then((scenario) => {
      ...
      instanceToUpdate.data = scenarioData.data // to trigger scenario listener broadcasting
  ...
  _scenarioListener (instance) {
    return (event, value) => {
      this.logger.info(`Scenario ${instance.data.name}: ${event}`)
      switch (event) {
        case 'triggered':
