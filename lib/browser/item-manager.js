...
const getFromId = (id, factories, returnFactory, settingPanelCallback = null) => {
  try {
    ...
    return { item: <div key={id}>Broken item! highlight_off warning healing</div> } // TODO !0: better error display
  }
}
...
class ItemManager {
  ...
  encapsulateItem ({ item, id, w = 1, h = 1, settingPanel = null }) {
    ...
    return (
      ...
      >
        {item}
        <Resizer acceptedDimensions={item.props.acceptedDimensions} mainComponent={this.mainComponent} itemId={id} />
      </Item>
    )
  }
